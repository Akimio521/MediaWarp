...
func Cache() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		cacheData, ok := cacheManager.GetCache("GIN-Cache", cacheKey) // 查询缓存记录
		logger.ServiceLogger.Debug("GIN-Cache：", "OK:", ok, ",KEY:", cacheKey)

		if ok { // 缓存存在，直接返回缓存数据
			responseCacheData = cacheData.(*ResponseCacheData)
			logger.ServiceLogger.Debug("GIN-Cache：缓存存在，直接返回缓存数据，HttpStatus:", responseCacheData.StatusCode)
			responseWithCache(ctx, responseCacheData)
		} else { // 缓存不存在，继续处理请求
			logger.ServiceLogger.Debug("GIN-Cache：缓存不存在，继续处理请求")
			...
			if code >= http.StatusOK && code < http.StatusMultipleChoices { // 响应是2xx的成功响应，更新缓存记录
				logger.ServiceLogger.Debug("GIN-Cache：创建缓存")
				...
				go cacheManager.UpdateCache("GIN-Cache", cacheKey, responseCacheData, cacheDuration)
			}
		}
	}
}
...
func getCacheTime(ctx *gin.Context) time.Duration {
	if ctx.Writer.Header().Get("Expired") == "-1" {
		logger.ServiceLogger.Debug("Expired=-1 => 不缓存")
		return 0
	}

	if ctx.Request.Body != nil && ctx.Request.ContentLength != 0 { // 请求体不为空
		logger.ServiceLogger.Debug("请求体不为空 => 不缓存")
		return 0
	...
	if ctx.Request.Method == http.MethodGet {
		...
		logger.ServiceLogger.Debug("请求方法不为GET => 不缓存")
		return 0
	}
}
